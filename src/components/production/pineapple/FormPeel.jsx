...
import {add_peel_pineapple} from "../../../redux/actions/production";


const FormPeel = ({close, pallets, slug, dispatch, id}) => {
    /*Formik*/
    const formik = useFormik({
        ...
        onSubmit: (form) => {
            form.process = id
            dispatch(add_peel_pineapple(form, slug), close())
        }
    })
    return (
        <div className="w-full z-20">
            <form className="bg-white px-8 pt-6 pb-8 mb-4">
                ...
                <div>
                    <p className={`${formik.errors.quantity ? "text-red-500" : "text-base mt-4 font-medium leading-none text-gray-800"}`}>Cantidad:</p>
                    <input
                        className="w-full p-3 mt-4 border border-gray-300 rounded outline-none focus:bg-gray-50 uppercase"
...
const newSchema = () => {
    return {
        ...
        quantity: Yup.number().min(1).max(42).required(),
    }
}
export default FormPeel;
